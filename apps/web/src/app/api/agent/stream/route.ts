...
export async function GET(req: Request) {
  ...
  const engine = (url.searchParams.get("engine") ?? "builtin").toLowerCase();

  if (engine === "proxy") {
    const upstream = url.searchParams.get("upstream") ?? process.env.AGENT_UPSTREAM_URL;
    if (!upstream) {
      return new Response("Missing AGENT_UPSTREAM_URL (or ?upstream=...)", { status: 400 });
    }
    ...
    if (!res.ok || !res.body) {
      return new Response(`Upstream error: HTTP ${res.status}`, { status: 502 });
    }
    ...
    start(controller) {
      ...
      (async () => {
        try {
          ...
          sendStep({ id: "browse", status: "done", detail: `已选择：${store.name}` });
